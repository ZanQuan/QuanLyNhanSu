...
namespace QLNhanSu.Forms
{
    public partial class frmMain : Form
    {
        ...
        {
            toolStripStatusLabel2.Text = "Người đăng nhập: " + SessionInfo.TenDangNhap
                                + "  |  Vai trò: " + SessionInfo.VaiTro;
            trangthai.Text = "Trạng thái: Sẵn sàng";

            HienThiAnhNen();

            //  CHẾ ĐỘ NHÂN VIÊN: chỉ hiện menu Chấm công + Bảng lương
        ...
        // Ẩn toàn bộ menu quản lý, chỉ giữ lại các mục nhân viên cần dùng.

        private void ApdungMenuNhanVien()
        {
            // Ẩn toàn bộ menu nghiệp vụ
            menubaomat.Visible = false;
            hồSơCNVToolStripMenuItem.Visible = false;   
            bảngDanhMụcToolStripMenuItem.Visible = false;   
            trợGiúpToolStripMenuItem.Visible = false;   
            báoCáoToolStripMenuItem1.Visible = false;   

            // Thêm nút Chấm công ca 
            var btnChamCong = new ToolStripButton
            {
                Text = "⏱  CHẤM CÔNG CA",
            ...
            btnChamCong.Click += (s, _) =>
            {
                frmChamCong frm = new frmChamCong();
                frm.ShowDialog();
            };

            var btnXemLuong = new ToolStripButton
            {
                Text = "💰  XEM BẢNG LƯƠNG",
                Font = new Font("Segoe UI", 11f, FontStyle.Bold),
            ...
            // Thêm vào MenuStrip 
            menuStrip1.Items.Insert(0, btnChamCong);
            menuStrip1.Items.Insert(1, btnXemLuong);
        ...
        private void hồSơĐiềuĐộngKThưởngKLuậtToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var itemHoSo = new ToolStripMenuItem("📁  Hồ sơ Điều Động & Nhân sự");
            itemHoSo.Click += (s, _) =>
            {
            ...
            var itemKTKL = new ToolStripMenuItem("⭐  Báo cáo Khen Thưởng - Kỷ Luật");
            itemKTKL.Click += (s, _) =>
            {
                frmBaoCaoKTKL frm = new frmBaoCaoKTKL();
                frm.ShowDialog();
            };

            menu.Items.Add(itemHoSo);
            menu.Items.Add(new ToolStripSeparator());
            menu.Items.Add(itemKTKL);
