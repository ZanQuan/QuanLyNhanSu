...
namespace QLNhanSu.Forms
{
    public partial class frmDMPhongBan : Form
    {
        ...
        private void dgv_DMPB_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv_DMPB.Rows[e.RowIndex].IsNewRow) return;
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            int soNV = Convert.ToInt32(DataProvider.ExecuteScalar(
        "SELECT COUNT(*) FROM NhanVien WHERE MaPB=@MaPB",
        new[] { new SqlParameter("@MaPB", txtmaPB.Text.Trim()) }));

            if (soNV > 0)
            {
                MessageBox.Show($"Không thể xóa! Phòng ban '{txttenPB.Text}' đang có {soNV} nhân viên.",
